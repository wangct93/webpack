...
const {aryNoRepeat,resolve,callFunc} = util;

const paths = require('./config/paths');

const {modelDir,pageDir,configOutputDir,componentDir} = paths;
...
async function updateModel(){
  ...
  const configOutputDirname = resolve(paths.configOutputDir);
  ...
  await babelContent(content).then(content => {
...
async function updateRouter(){
  ...
  await babelContent(configContent).then(content => {
...
function getRouterContent(option){
  ...
  return isRoot ? `import React from 'react';${aryNoRepeat(importList).join('')}export default ${content}` : content;
}
...
async function babelContent(content){
  return new Promise((cb,eb) => {
    const babel = require('@babel/core');
    const {getJsRule} = require('./config/common');
    const options = getJsRule().use[0].options;
