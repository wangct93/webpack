...
const {resolve,objectUtil,toArray} = util;
...
function getRules(build){
  ...
  const rules = [
    ...
    getJsRule({
      include:resolve('node_modules/wangct-webpack/entry')
    }),
    getEslintRule(),
    getFileRule(build),
    ...getCssRules(build),
...
function getCssRules(build){
  ...
  if(disableCssModules === true){
    ...
    const noCssModulePaths = toArray(disableCssModules).filter(item => item).concat(resolve('node_modules/antd'));
    return [
      getCssRule({
        build,
...
function getJsRule(option){
  ...
  return {
    ...
    ...objectUtil.clone(option,['exclude','include'])
  };
}
    ...
    use:[
      {
        ...
        options: {
          ...
          publicPath:build ? config.assetsPublicPath || './' : '/'
        }
      }
    ]
  }
