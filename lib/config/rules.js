...
function getJsRules(options = config){
  const rules = [getJsRule(options)];
  if(options.jsInclude){
    rules.push(getJsRule({
      ...options,
      include:options.jsInclude,
...
function getJsRule(options = config){
  ...
  return {
    ...
    exclude:[/node_modules/,...toAry(options.jsExclude)],
  };
}
 ...
 * 懒加载
 */
function getLibImportJsRule(options = config){
  const {libImport = true} = options;
  if(!libImport){
    return null;
  }
  return {
    test: /\.(ts|js)x?$/i,
    loader:resolveLib('loaders/lib-import'),
  };
}
