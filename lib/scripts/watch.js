...
var isSelf = process.env.isDvaSelf && process.env.isDvaSelf.trim() === '1';
var libSrcDirname = isSelf ? 'esSrc' : 'lib/src';
var modelDirname = resolveApp('src/models');
var configDirname = resolveApp('config');
var pageDirname = resolveApp('src/pages');
var modelOutputPath = resolve(libSrcDirname, 'config/models.js');
var componentConfigPath = resolveApp('src/components/config.js');
var routerOutputPath = resolve(libSrcDirname, 'config/routes.js');
...
function updateRouter(cb) {
  ...
  var output = resolve(tempDirname, 'config');
  new Babel({
    src: configDirname,
    output: output,
    success: function success() {
      ...
      var config = require(resolve(output, 'config'))["default"];
