...
module.exports = function(source){
  const re = /import\s+([^;]*?)\s+from\s+['"]@wangct\/react-lib['"];?/g;
  return toStr(source).replace(re,(match,first) => {
    ...
    const replaceImportList = list.map((modName) => {
      ...
      if(isUpper){
        let importStr = `import ${modName} from '@wangct/react-lib/lib/${modName}';`;
        const cssPath = pathResolve(`node_modules/@wangct/react-lib/lib/${modName}/index.less`);
        if(fileIsExist(cssPath)){
          importStr += `\nimport '@wangct/react-lib/lib/${modName}/index.less';`;
        }
        return importStr;
      }
      return `import {${modName}} from '@wangct/react-lib/lib/utils/exportUtil';`;
    });
